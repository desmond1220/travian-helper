...
(function (CurrentPageEnum) {
  ...
  CurrentPageEnum["VILLAGE_POSITION"] = "VILLAGE_POSITION";
...
class StateHandler {
  constructor() {
    this.parseState = (prop) => {
      ...
      if (item === null) return StateHandler.INITIAL_STATE[prop];
    ...
    this.state = Object.fromEntries(
      Object.keys(StateHandler.INITIAL_STATE).map((k) => [
        k,
        this.parseState(k),
      ])
    );
...
Utils.addToDate = (date, hour, minute, second) => {
  return new Date(
    date.getTime() + hour * 60 * 60 * 1000 + minute * 60 * 1000 + second * 1000
  );
};
...
Utils.formatDate = (dateInput) => {
  ...
  return `${date.getFullYear()}/${Utils.leftPadZero(
    date.getMonth() + 1,
    2
  )}/${Utils.leftPadZero(date.getDate(), 2)} ${Utils.leftPadZero(
    date.getHours(),
    2
  )}:${Utils.leftPadZero(date.getMinutes(), 2)}:${Utils.leftPadZero(
    date.getSeconds(),
    2
  )}`;
};
Utils.isSufficientResources = (required, own) => {
  return (
    required.lumber <= own.lumber &&
    required.clay <= own.clay &&
    required.iron <= own.iron &&
    required.crop <= own.crop
  );
};
class Navigation {}
_b = Navigation;
Navigation.goToVillage = (state, id, action) =>
  __awaiter(void 0, void 0, void 0, function* () {
    yield Utils.delayClick();
    state.currentAction = action;
    state.feature.debug &&
      console.log(`Go to village - [${id}]${state.villages[id].name}`);
    $(`.listEntry[data-did="${id}"] > a`)[0].click();
    return true;
  });
Navigation.goToBuilding = (state, aid, gid, action) =>
  __awaiter(void 0, void 0, void 0, function* () {
    if (aid <= 18 && state.currentPage === CurrentPageEnum.FIELDS) {
      ...
      state.feature.debug &&
        console.log(
          `Go to building - [aid=${aid},gid=${gid}]${GID_NAME_MAP[gid]}`
        );
      ...
      state.feature.debug &&
        console.log(
          `Cannot go to building - [aid=${aid},gid=${gid}]${GID_NAME_MAP[gid]}`
        );
      return false;
    }
  });
Navigation.goToFields = (state, action) =>
  __awaiter(void 0, void 0, void 0, function* () {
    yield Utils.delayClick();
    state.currentAction = action;
...
Navigation.goToTown = (state, action) =>
  __awaiter(void 0, void 0, void 0, function* () {
    yield Utils.delayClick();
    state.currentAction = action;
...
const updateVillageList = (state) => {
  ...
  const currentVillageId = villageListEle
    .filter((_, ele) => ele.className.includes("active"))
    .attr("data-did");
  const villiageIds = [];
  villageListEle.each((index, ele) => {
    var _c, _d, _e;
    const id =
      (_c = ele.attributes.getNamedItem("data-did")) === null || _c === void 0
        ? void 0
        : _c.value;
    if (!id) {
      return;
    ...
    const x = Utils.parseIntIgnoreNonNumeric(
      ((_d = coordinateAttributes.getNamedItem("data-x")) === null ||
      _d === void 0
        ? void 0
        : _d.value) || ""
    );
    const y = Utils.parseIntIgnoreNonNumeric(
      ((_e = coordinateAttributes.getNamedItem("data-y")) === null ||
      _e === void 0
        ? void 0
        : _e.value) || ""
    );
    ...
    villages[id] = Object.assign(
      Object.assign(Object.assign({}, villageDefaults), villages[id]),
      { id, name, index, position: { x, y } }
    );
  });
  state.villages = Object.fromEntries(
    Object.entries(villages).filter(([id, _]) => villiageIds.includes(id))
  );
  if (currentVillageId) state.currentVillageId = currentVillageId;
};
const updateCurrentVillageStatus = (state) => {
  ...
  const warehouseCapacity = Utils.parseIntIgnoreNonNumeric(
    $(".warehouse .capacity > div").text()
  );
  const granaryCapacity = Utils.parseIntIgnoreNonNumeric(
    $(".granary .capacity > div").text()
  );
  ...
  if (state.currentPage === CurrentPageEnum.FIELDS) {
    ...
    $("#movements tr").each((_, ele) => {
      ...
      if (!typeEle.length) return;
      const type =
        (_c = typeEle[0].attributes.getNamedItem("class")) === null ||
        _c === void 0
          ? void 0
          : _c.value;
...
const alertAttack = (state) => {
  ...
  const attack = village.incomingTroops.find(
    (e) => e.type === TroopMovementType.ATTACK
  );
  if (attack) {
    ...
    if (
      !village.attackAlertBackoff ||
      new Date(village.attackAlertBackoff) < new Date()
    ) {
      state.feature.debug &&
        console.log(`Send alert for attack at village ${village.name}`);
      village.attackAlertBackoff = Utils.addToDate(new Date(), 0, 5, 0);
      state.villages = villages;
      fetch(
        `https://api.telegram.org/bot${
          state.telegramToken
        }/sendMessage?chat_id=${state.telegramChatId}&text=Village ${
          village.name
        } under attack at ${Utils.formatDate(attack.time)}`
      );
    } else {
      state.feature.debug &&
        console.log(
          `Not alerting attack due to backoff at ${Utils.formatDate(
            village.attackAlertBackoff
          )}`
        );
    }
...
const alertEmptyBuildQueue = (state) => {
  ...
  if (!village.currentBuildTasks.length) {
    ...
    if (
      ...
      new Date(village.emptyBuildQueueAlertBackoff) < new Date()
    ) {
      state.feature.debug &&
        console.log(`Send alert for attack at village ${village.name}`);
      village.emptyBuildQueueAlertBackoff = Utils.addToDate(
        new Date(),
        0,
        5,
        0
      ...
      state.villages = villages;
      fetch(
        `https://api.telegram.org/bot${state.telegramToken}/sendMessage?chat_id=${state.telegramChatId}&text=Village ${village.name} build queue is empty`
      );
    } else {
      state.feature.debug &&
        console.log(
          `Not alerting empty build queue due to backoff at ${Utils.formatDate(
            village.emptyBuildQueueAlertBackoff
          )}`
        );
    }
...
const alertResourceCapacityFull = (state) => {
  ...
  if (fullResourceType) {
    ...
    if (
      !village.resourceCapacityFullAlertBackoff ||
      new Date(village.resourceCapacityFullAlertBackoff) < new Date()
    ) {
      state.feature.debug &&
        console.log(
          `Send alert for capacity full for ${fullResourceType} at village ${village.name}`
        );
      village.resourceCapacityFullAlertBackoff = Utils.addToDate(
        new Date(),
        0,
        5,
        0
      ...
      state.villages = villages;
      fetch(
        `https://api.telegram.org/bot${state.telegramToken}/sendMessage?chat_id=${state.telegramChatId}&text=Village ${village.name} ${fullResourceType} is at capacity`
      );
    } else {
      state.feature.debug &&
        console.log(
          `Not alerting capacity full due to backoff at ${Utils.formatDate(
            village.resourceCapacityFullAlertBackoff
          )}`
        );
    }
...
const build = (state) =>
  __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (village.pendingBuildTasks.length > 0) {
      ...
      if (
        village.currentBuildTasks.length < 2 &&
        [CurrentPageEnum.FIELDS, CurrentPageEnum.TOWN].includes(
          state.currentPage
        ) &&
        Utils.isSufficientResources(task.resources, village.resources)
      ) {
        const success = yield Navigation.goToBuilding(
          state,
          task.aid,
          task.gid,
          CurrentActionEnum.BUILD
        );
        if (!success) {
          if (state.currentPage === CurrentPageEnum.FIELDS)
            yield Navigation.goToTown(state, CurrentActionEnum.BUILD);
          else yield Navigation.goToFields(state, CurrentActionEnum.BUILD);
        }
        return;
      }
      ...
      if (
        state.currentPage === CurrentPageEnum.BUILDING &&
        params.get("id") === `${task.aid}` &&
        params.get("gid") === `${task.gid}`
      ) {
    ...
    const nextVillageIdToBuild = Object.entries(state.villages)
      .filter(
        ([_, village]) =>
          ...
          village.currentBuildTasks.filter(
            (task) => new Date(task.finishTime) > new Date()
          ).length < 2 &&
          Utils.isSufficientResources(
            village.pendingBuildTasks[0].resources,
            village.resources
          )
      )
      .map(([id, _]) => id)
...
const nextVillage = (state) =>
  __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (nextRotationTime < new Date()) {
      state.nextVillageRotationTime = Utils.addToDate(
        new Date(),
        0,
        Utils.randInt(5, 10),
        Utils.randInt(0, 59)
      ...
      Object.values(state.villages).forEach((village) => {
        var _c;
        const earliestUpdatedTime =
          (_c = state.villages[earliestVillageId]) === null || _c === void 0
            ? void 0
            : _c.lastUpdatedTime;
        if (
          !village.lastUpdatedTime ||
          (earliestUpdatedTime && village.lastUpdatedTime < earliestUpdatedTime)
        ) {
          earliestVillageId = village.id;
        }
      });
      state.feature.debug &&
        console.log(`Rotating to ${state.villages[earliestVillageId].name}`);
      yield Navigation.goToVillage(
        state,
        earliestVillageId,
        CurrentActionEnum.NAVIGATE_TO_FIELDS
      ...
      state.feature.debug &&
        console.log(
          `Not rotating, next rotation=${Utils.formatDate(
            nextRotationTime
          )}, current=${Utils.formatDate(currentTime)}`
        );
    }
  });
...
const render = (state) => {
  ...
  $("#console").html(`
        <div class="flex-row">
            <h4>Console</h4>
            <input id="toggleAutoLogin" class="ml-5" type="checkbox" ${
              state.feature.autoLogin ? "checked" : ""
            }/> Auto login
            <input id="toggleAutoScan" class="ml-5" type="checkbox" ${
              state.feature.autoScan ? "checked" : ""
            }/> Auto scan
            <input id="toggleAutoBuild" class="ml-5" type="checkbox" ${
              state.feature.autoBuild ? "checked" : ""
            }/> Auto build
            <input id="toggleAutoFarm" class="ml-5" type="checkbox" ${
              state.feature.autoFarm ? "checked" : ""
            }/> Auto farm
            <input id="toggleAutoFarmWithHero" class="ml-5" type="checkbox" ${
              state.feature.autoFarmWithHero ? "checked" : ""
            }/> Auto farm with hero
            <input id="toggleAlertAttack" class="ml-5" type="checkbox" ${
              state.feature.alertAttack ? "checked" : ""
            }/> Alert attack
            <input id="toggleAlertEmptyBuildQueue" class="ml-5" type="checkbox" ${
              state.feature.alertEmptyBuildQueue ? "checked" : ""
            }/> Alert empty build queue
            <input id="toggleAlertResourceCapacityFull" class="ml-5" type="checkbox" ${
              state.feature.alertResourceCapacityFull ? "checked" : ""
            }/> Alert resource capacity full
            <input id="toggleDebug" class="ml-5" type="checkbox" ${
              state.feature.debug ? "checked" : ""
            }/> Debug
        </div>
        <div>
            ...
            <div>Current Page: ${
              state.currentPage
            } (Last render: ${Utils.formatDate(new Date())})</div>
            <div>Current Action: ${state.currentAction}</div>
            <div>Next rotation: ${Utils.formatDate(
            ...
            ${Object.entries(villages)
              .map(
                ...
                <div class="village-container">
                    <h4>${village.name} (id: ${id}) (${village.position.x}, ${
                  village.position.y
                })</h4>
                    <div>Last update: ${Utils.formatDate(
                      village.lastUpdatedTime
                    )}</div>
                    <div>Attack alert backoff: ${Utils.formatDate(
                      village.attackAlertBackoff
                    )}</div>
                ...
                <br/>
                    ...
                    ${village.currentBuildTasks
                      .map(
                        (task) => `
                        <div>${task.name} ${task.level} ${Utils.formatDate(
                          task.finishTime
                        )}</div>
                    `
                      )
                    ...
                    ${village.incomingTroops
                      .map(
                        (troop) => `
                        <div>${troop.type} ${troop.count} ${Utils.formatDate(
                          troop.time
                        )}</div>
                    `
                      )
  ...
  state.currentPage === CurrentPageEnum.BUILDING &&
    $("#addCurrentToPending").on("click", () => {
      ...
      const pendingBuildTasks =
        villages[state.currentVillageId].pendingBuildTasks;
      const params = new URLSearchParams(window.location.search);
      ...
      const lumber = Utils.parseIntIgnoreNonNumeric(
        resourceRequirementEle[0].innerText
      );
      const clay = Utils.parseIntIgnoreNonNumeric(
        resourceRequirementEle[1].innerText
      );
      const iron = Utils.parseIntIgnoreNonNumeric(
        resourceRequirementEle[2].innerText
      );
      const crop = Utils.parseIntIgnoreNonNumeric(
        resourceRequirementEle[3].innerText
      );
  ...
  $(".removeFromPending").on("click", (ele) => {
    var _c;
    const idx =
      (_c = ele.target.attributes.getNamedItem("idx")) === null || _c === void 0
        ? void 0
        : _c.value;
    if (!idx) return;
    const villages = state.villages;
  ...
  handleFeatureToggle("#toggleAutoFarmWithHero", state, "autoFarmWithHero");
  handleFeatureToggle("#toggleAlertAttack", state, "alertAttack");
  handleFeatureToggle(
    "#toggleAlertEmptyBuildQueue",
    state,
    "alertEmptyBuildQueue"
  );
  handleFeatureToggle(
    "#toggleAlertResourceCapacityFull",
    state,
    "alertResourceCapacityFull"
  );
...
const run = (state) =>
  __awaiter(void 0, void 0, void 0, function* () {
    while (true) {
      ...
      if (
        [CurrentPageEnum.LOGIN].includes(state.currentPage) &&
        state.feature.autoLogin
      ) {
      ...
      if (
        [
          CurrentPageEnum.FIELDS,
          CurrentPageEnum.TOWN,
          CurrentPageEnum.BUILDING,
        ...
        if (
          [CurrentActionEnum.IDLE, CurrentActionEnum.BUILD].includes(
            state.currentAction
          ) &&
          state.feature.autoBuild
        ) {
        ...
        if (CurrentActionEnum.NAVIGATE_TO_FIELDS === state.currentAction) {
          if (state.currentPage === CurrentPageEnum.FIELDS)
            state.currentAction = CurrentActionEnum.IDLE;
          else yield Navigation.goToFields(state, CurrentActionEnum.IDLE);
        }
        if (
          [CurrentActionEnum.IDLE, CurrentActionEnum.FARM].includes(
            state.currentAction
          ) &&
          state.feature.autoFarm
        ) {
